...
of shhh, so `shhh-docs:latest` is republished between releases, alongside a `sha-<commit>` tag that
never moves. The version tags are written only by a release and never rewritten, so `shhh-docs:1.1.9`
