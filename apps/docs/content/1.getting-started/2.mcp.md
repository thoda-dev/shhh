...
::warning
Claude and ChatGPT call the server from their own infrastructure, so they need a URL that is
publicly reachable over HTTPS. Cursor, VS Code and Claude Code connect from your own machine, where
`http://localhost:3000/mcp` works fine.
::
